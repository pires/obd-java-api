...
public abstract class ObdCommand {
	 ...
	 */
	private static final Class<?>[] ERROR_CLASSES = { UnableToConnectException.class, BusInitException.class,
			MisunderstoodCommandException.class, NoDataException.class, StoppedException.class,
			UnknownErrorException.class, UnsupportedCommandException.class };
	...
	protected void readRawData(InputStream in) throws IOException {
		 ...
		 * attention to the fact that I've put the word byte in quotes, because 41 is
		 * actually TWO bytes (two chars) in the socket. So, we must do some more
		 * processing..
		 */
		rawData = removeAll(SEARCHING_PATTERN, res.toString());

		/*
		 * Data may have echo or informative text like "INIT BUS..." or similar. The
		 * response ends with two carriage return characters. So we need to take
	...
	void checkForErrors() {
		for (Class<?> errorClass : ERROR_CLASSES) {
	...
	/**
	 * Set to 'true' if you want to use imperial units, false otherwise. By default
